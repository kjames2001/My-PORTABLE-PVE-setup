...
## Mount an external ssd/microsd card on the pve host using fstab.
I mount a 4tb 2280 ssd to the proxmox host for all media file storage. Still portable and the only thing you need to do is make sure the ssd is plugged in before you power the machine. Alternatively, you could use the micro sd card slot with a 2tb micro sd card for the same purpose, just slower.

Plug in the ssd, open proxmox shell.
Find the ssd:
```
lsblk
```
...
├─sda1                         8:1    1     6G  0 part 
└─sda2                         8:2    1 466.7G  0 part 
sdb                            8:16   0   3.7T  0 disk 
├─sdb1                         8:17   0    16M  0 part 
└─sdb2                         8:18   0   3.7T  0 part /mnt/docker
...
└─nvme0n1p3                  259:3    0   1.9T  0 part 
  ...
  │   └─pve-vm--103--disk--0 252:7    0    20G  0 lvm  
  └─pve-data_tdata           252:3    0   1.7T  0 lvm  
    └─pve-data-tpool         252:4    0   1.7T  0 lvm  
      ...
      └─pve-vm--103--disk--0 252:7    0    20G  0 lvm 
```
Note your ssd dev. for me, its /dev/sdb. But you would want to mount the partition, not the dev, so its /dev/sdb2 for me.
Find the block id for the partition:
```
blkid /dev/sdb2
```
should give you something like this:
```
/dev/sdb2: UUID="4C4D-5C3A" BLOCK_SIZE="512" TYPE="exfat" PARTLABEL="Basic data partition" PARTUUID="74bb65a3-5406-44b8-bb9b-86767e8fccd7"
```
note the uuid, in my case its: 4C4D-5C3A.
...
and add this line:
```
UUID=4C4D-5C3A /mnt/docker exfat defaults,nofail,x-systemd.device-timeout=10s 0 2
```
note to change exfat to your ssd partition format and change /mnt/docker to whatever dir you would like to mount your ssd.

Create mount point:
```
mkdir -p /mnt/docker
```
change /mnt/docker to whatever dir you would like to mount your ssd.
